...
def ensure_file_downloaded(
    ...
    if with_param:
        shell([downloader_executable, "-U", "NoSuchBrowser/1.0", source_url, "-O", tmp_path])
    else:
    ...
    if unpack:
        ...
        elif unpack_type == "un7z":
            shell(["7z", "x", "-y", tmp_path, "-o" + tmp2_path])
        else:
