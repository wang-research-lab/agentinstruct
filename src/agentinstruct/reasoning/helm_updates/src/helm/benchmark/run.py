...
from .run_expander import parse_helm_name
...
def run_entries_to_run_specs(
    ...
    skip_expander: Optional[bool] = False,
) -> List[RunSpec]:
    ...
    for entry in run_entries:
        ...
        for run_spec in construct_run_specs(parse_object_spec(entry.description), skip_expander):
            ...
            if skip_expander:
                prompt_list = {"dataset_name": parse_helm_name(run_spec.name).replace(':', '_')}
                adapter_spec = replace(
                    adapter_spec, prompt_list = prompt_list
                )
                run_spec = replace(run_spec, adapter_spec=adapter_spec)
...
def add_run_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--skip-expander",
        action="store_true",
        help="Bypass instruction expanders when constructing instances",
    )
...
def main():
    ...
    run_specs = run_entries_to_run_specs(
        ...
        skip_expander=args.skip_expander,
    )
