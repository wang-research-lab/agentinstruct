...
@run_spec_function("multi_arith")
def get_multi_arith_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.multi_arith_scenario.MultiArithScenario", args={})
...
@run_spec_function("aqua")
def get_aqua_spec(method: str = ADAPT_MULTIPLE_CHOICE_JOINT) -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.aqua_scenario.AQuAScenario", args={})
...
@run_spec_function("svamp")
def get_svamp_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.svamp_scenario.SVAMPScenario", args={})
...
@run_spec_function("singleeq")
def get_singleeq_arith_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.singleeq_scenario.SingleEqScenario", args={})
...
@run_spec_function("coin")
def get_coin_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.coin_scenario.CoinScenario", args={})

    adapter_spec = get_generation_adapter_spec(
        input_noun="Question",
        output_noun="Answer (yes or no)",
        max_tokens=4,
        stop_sequences=None,
    )
...
@run_spec_function("letter")
def get_letter_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.letter_scenario.LetterScenario", args={})
...
@run_spec_function("big_bench_hard")
def get_big_bench_hard_spec(dataset: str) -> RunSpec:
    scenario_spec = ScenarioSpec(
        class_name="helm.benchmark.scenarios.big_bench_hard_scenario.BigBenchHardScenario",
        args={"dataset": dataset},
    )
    method = ADAPT_GENERATION
    adapter_spec = get_generation_adapter_spec(
    ...
    metric_specs = get_exact_match_metric_specs() + get_open_ended_generation_metric_specs() + get_generative_harms_metric_specs()
    return RunSpec(
        name=f"big_bench_hard:dataset={dataset},method={method}",
        scenario_spec=scenario_spec,
        adapter_spec=adapter_spec,
        metric_specs=metric_specs,
        groups=["big_bench_hard"],
    )
    
@run_spec_function("commonsense_qa")
def get_commonsense_qa_spec() -> RunSpec:
    scenario_spec = ScenarioSpec(class_name="helm.benchmark.scenarios.commonsense_qa_scenario.CommonsenseQAScenario", args={})
    ...
    return RunSpec(
        name=f"commonsense_qa:method={ADAPT_MULTIPLE_CHOICE_JOINT}",
        scenario_spec=scenario_spec,
        adapter_spec=adapter_spec,
        metric_specs=metric_specs,
        groups=["commonsense_qa"],
    )
...
def get_xsum_summarization_spec(temperature: float = 0.3, device: str = "cpu") -> RunSpec:
    ...
    return RunSpec(
        ...
        scenario_spec=scenario_spec,
        adapter_spec=adapter_spec,
        metric_specs=get_basic_metric_specs(['rouge_2']),
...
def construct_run_specs(spec: ObjectSpec, skip_expander: Optional[bool]) -> List[RunSpec]:
    """
    ...
    for expander in expanders:
        if skip_expander and isinstance(expander, InstructionsRunExpander):
            continue
        run_specs = [
