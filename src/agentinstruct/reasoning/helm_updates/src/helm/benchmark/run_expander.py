...
def parse_helm_name(name):
    """Given a name, will parse the name to be used in the dict."""
    description = name  # item["description"]
    rest_of_description = re.sub(r"model=.*?,", "", description)
    rest_of_description = re.sub(r"(,|:)model=.*?(,|$)", "", description)
    rest_of_description = re.sub(r",?max_train_instances=.*?(,|$)", "", rest_of_description)
    rest_of_description = re.sub(r"(,|:)(level|k)=.*?(,|$)", ",", rest_of_description)
    rest_of_description = re.sub(r"(,|:)(device|timeout|temperature|method)=.*?$", "", rest_of_description)
    rest_of_description = re.sub(r",use_official_examples=False", "", rest_of_description)
    rest_of_description = re.sub(r",use_chain_of_thought=False", "", rest_of_description)
    rest_of_description = re.sub(r"_sampled", "", rest_of_description)
    rest_of_description = re.sub(r"(:|_)num_parenthesis_pairs", "_np", rest_of_description)
    rest_of_description = re.sub(r"disinformation:capability", "disinfo:type", rest_of_description)

    # dataset phrase (to be put in agentinstruct)
    dataset_phrase = re.sub(r"(^(.*?):)", r"The dataset name is \1", rest_of_description)
    pattern = r"(,|:)(.*?)=(.*?)(,|$)"
    while re.search(pattern, dataset_phrase) is not None:
        dataset_phrase = re.sub(pattern, r" and the \2 is \3,", dataset_phrase)
    # Remove final comma/colon and add to dict.
    rest_of_description = re.sub(r":$", "", rest_of_description)
    dataset_phrase = re.sub(r"(,|:)$", "", dataset_phrase)
    return rest_of_description
...
class InstructionsRunExpander(RunExpander):
    ...
    def expand(self, run_spec: RunSpec) -> List[RunSpec]:
        ...
        if self.value == "none":
        ...
        elif self.value == 'agentinstruct':
            
            prompt_list = {
                'agentinstruct': True,
                'dataset_name': parse_helm_name(run_spec.name).replace(':', '_'),
                'instructions': None,
                'task': None,
            ...
            with open('instructions/_latest/instructions.json', 'r') as f:
                instructions_dict = json.load(f)
            f.close()
            instructions_dict = {parse_helm_name(k): v for k, v in instructions_dict.items()}
            prompt_list['task'] = instructions_dict[prompt_list['dataset_name']]['task']
            prompt_list['instructions'] = instructions_dict[prompt_list['dataset_name']]['instructions']
            adapter_spec = replace(
                adapter_spec,
                instructions='', 
            ) 
