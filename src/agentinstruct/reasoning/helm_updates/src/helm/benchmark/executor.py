...
class Executor:
    ...
    def execute(self, scenario_state: ScenarioState) -> ScenarioState:
        ...
        # Fill in process with prompt list (accessible from ScenarioState) so it only has one variable
        process = lambda x: self.process(x, scenario_state.adapter_spec.prompt_list)
    ...
    def process(self, state: RequestState, prompt_list: Dict[str, Any]) -> RequestState:
        try:
            result: RequestResult = self.service.make_request(self.execution_spec.auth, state.request, prompt_list)
