...
class InContextLearningAdapter(Adapter, ABC):
    ...
    def _make_prompt_fit(self, prompt: Prompt) -> Prompt:
        ...
        text = prompt.text
        prompt_list = self.adapter_spec.prompt_list
        
        if 'agentinstruct' in prompt_list:
        
            if prompt_list['agentinstruct']:
                p1 = "Now apply the instruction steps to generate an explanation that reasons (in a paragraph) towards the correct answer to the task above. Do not start out by saying your answer as you want the reasoning to guide you. End the explanation with the correct answer. After you have the answer, write DONE.\n\nExplanation: "
                p0 = "You will be provided instructions for completing a task followed by a task to complete.\n\n" + self.adapter_spec.prompt_list['instructions'] + "\n\n" + "\n" + p1
                tokens = int(0.25 * len(p0)) + 512 + self.adapter_spec.max_tokens
            
            else: #zeroshot-cot
                ins_length = len(prompt_list['instructions']) if prompt_list['instructions'] else 0
                tokens = int(0.25 * ins_length  + 0.25 * len("Let's think step by step.")) + 512 + self.adapter_spec.max_tokens
        
        else: tokens = self.adapter_spec.max_tokens
        
        truncated_text = self.window_service.truncate_from_right(text, tokens)
