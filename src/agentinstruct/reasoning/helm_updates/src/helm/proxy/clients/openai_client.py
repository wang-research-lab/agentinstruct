...
from .openai_automatic_prompt_tuning import trigger_word_pipeline
    ...
    def make_request(self, request: Request, prompt_list: Dict[str, Any]) -> RequestResult:
        ...
        if request.embedding:
            ...
            raw_request = {
                ...
                "max_tokens": 1 if request.max_tokens == 0 else request.max_tokens,
        ...
        if True: #try:
            if request.embedding:
                ...
                def do_it():
                    ...
                    raw_request["messages"] = request.prompt 
               
                    return trigger_word_pipeline(raw_request, prompt_list.copy())                    
            ...
            # NOTE: avoiding the cache bc that's what's making it not run. do not know why.
            #with htrack_block("Baseline Response"):
            response, full_text = do_it()
            #    hlog(f"Response: {response}")
            cached = False
            response["request_time"] = 0
            #raise ValueError(f'out: {out}')
    ...
    def tokenize(self, request: TokenizationRequest) -> TokenizationRequestResult:
        ...
        try:
            ...
            def do_it():
                ...
                tokens = tokenizer.encode(request.text, disallowed_special=())
