...
class AutoClient(Client):
    ...
    def _get_client(self, model: str) -> Client:
        ...
        if client is None:
            ...
            elif organization == "together" or organization == "togethercomputer" or organization == "lmsys":
                client = TogetherClient(api_key=self.credentials.get("togetherApiKey", None), cache_config=cache_config)
            elif organization == "local":
                client = LocalClient(cache_config=cache_config)
    ...
    def make_request(self, request: Request, prompt_list: Dict[str, Any]) -> RequestResult:
        ...
        @retry_request
        def make_request_with_retry(client: Client, request: Request, prompt_list: Dict[str, Any]) -> RequestResult:
            return client.make_request(request, prompt_list)

        client: Client = self._get_client(request.model)

        try:
            return make_request_with_retry(client=client, request=request, prompt_list=prompt_list)
